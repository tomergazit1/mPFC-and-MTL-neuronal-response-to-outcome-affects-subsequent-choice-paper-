...
STDsToRemove=0;  
ApearAchieveOrMove='Appear' % 1 for appear, 2 for achieve and 3 for move
doContrasts=0;
BalanceNforAvoid=0;
BalanceNforCue=0;
...
%     for unit=[272:272]
        ...
        counterCond=0;
        for condA=1:size(conds,1) % the following loop is to enquire which condition has the minimal number of trials. then in the next loop you make the other conditions with the same number of trials as this one
            for condB=1:size(conds,1)
                counterCond=counterCond+1;
                CondLen(counterCond)=eval(['length(PSTHstruct(unit).' ApearAchieveOrMove '.' conds{condA,condB} '.timesInMS)']);
            end;
        end;
        minNumTrials=min(CondLen);
            
        for cond=1:size(conds,1)
            rasterTempWindowed=[];
            fprintf(['condition: ' conds{cond,1} '_' conds{cond,2} '\n']);

%             PSTHtemp1=eval(['PSTHstruct(unit).' ApearAchieveOrMove '.' conds{cond,1} '.PSTH']);
            rasterTemp1=eval(['PSTHstruct(unit).' ApearAchieveOrMove '.' conds{cond,1} '.raster']);
            if size(rasterTemp1,1)>minNumTrials
                rasterTemp1=rasterTemp1(randperm(size(rasterTemp1,1),minNumTrials),:);%%%
            end;
                
            
%             PSTHtemp2=eval(['PSTHstruct(unit).' ApearAchieveOrMove '.' conds{cond,2} '.PSTH']);
            rasterTemp2=eval(['PSTHstruct(unit).' ApearAchieveOrMove '.' conds{cond,2} '.raster']);
            if size(rasterTemp2,1)>minNumTrials
                rasterTemp2=rasterTemp2(randperm(size(rasterTemp2,1),minNumTrials),:);%%%
            end;
            
            rasterTemp=[rasterTemp1;rasterTemp2];
            %%%%%%%%%%%%%% special treatment for approach avoid comparison
            %%%%%%%%%%%%%% because there are many more approach
            if BalanceNforAvoid
                if strcmp(conds{cond,1},'ApproachRisk')%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              if BalanceNforCue % In Appears, we want to deal with unbalanced event number so we reduce pun events to match reward
                if cond==2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ll=length(PSTHstruct(unit).Appear.UncontrolledGainAppears.timesInMS);%%%%%%%%%
                    rasterTempRand=rasterTemp(randperm(size(rasterTemp,1),ll),:);%%%
                    rasterTemp=rasterTempRand;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                end;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            end;
            ...
            for perm=1:Niter
             ...
             eval(['PSTHstruct(unit).' ApearAchieveOrMove '.'  conds{cond,1} '_' conds{cond,2} '.Boot.p=' num2str(p) ';']);
             eval(['PSTHstruct(unit).' ApearAchieveOrMove '.'  conds{cond,1} '_' conds{cond,2} '.Boot.windows=[' num2str(relevantWindows) '];']);
             ...
             for wind=1:length(relevantWindows)
                ...
                if STDsToRemove>0 % deal with outliers
                    ...
                    fprintf(['for unit ' num2str(unit) ' condition '  conds{cond,1} '_' conds{cond,2} ' there are ' num2str(length(outlierUnder)+ length(outlierOver)) ' outliers\n' ]);
                end;
            ...
            eval(['PSTHstruct(unit).' ApearAchieveOrMove '.'  conds{cond,1} '_' conds{cond,2} '.Boot.sig=' num2str(signifCond(cond,1)) ';'])
        end;
